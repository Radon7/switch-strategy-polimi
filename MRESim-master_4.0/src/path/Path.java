...
public class Path {
    ...
    public static LinkedList<Point> getCellsAround(OccupancyGrid agentGrid, Point p){
    
        LinkedList<Point> l = new LinkedList<>();
        if(agentGrid.directLinePossible3(p.x, p.y, p.x + 1, p.y)){
            l.add(new Point(p.x + 1, p.y));
        }
        
        if(agentGrid.directLinePossible3(p.x, p.y, p.x, p.y + 1)){
    ...
    public boolean getAStarPath(OccupancyGrid agentGrid, Point startpoint, Point endpoint, boolean limit)
    {
        ...
        while (!openSet.isEmpty())
        {
            ...
            int current_index = getLowestScoreInList(openSet, f_score);
            current = openSet.get(current_index);
            if (current.distance(goal) < Constants.STEP_SIZE && grid.directLinePossible(goal.x, goal.y, current.x, current.y) && grid.directLinePossible(current.x, current.y, goal.x, goal.y))
            {
                
                if(!goal.equals(current))
                    came_from.put(goal, current);
                
                length = g_score.get(current) + current.distance(goal);
    ...
    public boolean getJumpPath(OccupancyGrid agentGrid, Point startpoint, Point endpoint, boolean limit)
    {
        ...
        /* ADDED TO ReconstructJumpPath(HashMap<Point, Point> came_from, Point current_node)
 ...
 */
    ...
    private LinkedList<Point> jump_neighbours(Point pt, Point parent) {
        ...
        if (dx != 0 && dy != 0) {
            ...
            if (!(grid.locationExists(nodeX - dx, nodeY) && grid.freeSpaceAt(nodeX - dx, nodeY)) && 
                    (grid.locationExists(nodeX, nodeY + dy) && grid.freeSpaceAt(nodeX, nodeY + dy))) {
                validNeighbours.add(new Point(nodeX - dx, nodeY + dy));
            }
            if (!(grid.locationExists(nodeX, nodeY - dy) && grid.freeSpaceAt(nodeX, nodeY - dy)) && 
                   (grid.locationExists(nodeX + dx, nodeY) && grid.freeSpaceAt(nodeX + dx, nodeY))) {
        ...
        // search horizontally/vertically
            ...
            if(dx == 0) {
                if (grid.locationExists(nodeX, nodeY + dy) && grid.freeSpaceAt(nodeX, nodeY + dy)) {
                    ...
                    if (!(grid.locationExists(nodeX + 1, nodeY) && grid.freeSpaceAt(nodeX + 1, nodeY))) {
                ...
                if (grid.locationExists(nodeX + dx, nodeY) && grid.freeSpaceAt(nodeX + dx, nodeY)) {
                    ...
                    if (!(grid.locationExists(nodeX, nodeY + 1) && grid.freeSpaceAt(nodeX, nodeY + 1))) {
            ...
            HashMap<Integer, TopologicalNode> topologicalNodes)
    {
        ...
        /*ADDED in ReconstructPath(HashMap<Point, Point> came_from, Point current_node)
        if (reversePathPoints != null) {
            Iterator<Point> i = reversePathPoints.iterator();
            Point curr, last = start;
            length = 0;
    ...
    {
        Point last_node; // The child node
        // From goal to start, but the elements are added to the head the list is already ordered appropriately
        while (came_from.containsKey(current_node) && (came_from.get(current_node) != current_node))
        {
            pathPoints.add(0, current_node);
            last_node = current_node;
            current_node = came_from.get(current_node); // the parent node
            allPathPixels = mergeLists(allPathPixels, pointsAlongSegment(last_node.x, last_node.y, current_node.x, current_node.y));
        }
        pathPoints.add(0,current_node);
                ...
                if (pts.get(0).distance(current_node) < pts.get(pts.size() - 1).distance(current_node)) //points are in reverse
                {
                    for (int i = pts.size() - 2; i >=0; i--)
                    {
                        ...
                        //        && (pathPoints.get(pathPoints.size() - 1).distance(pts.get(i)) <= 2))
                            allPathPixels = mergeLists(allPathPixels, pointsAlongSegment(pathPoints.get(0).x, pathPoints.get(0).y, pts.get(i).x, pts.get(i).y));
                            length += pts.get(i).distance(pathPoints.get(0));
                            pathPoints.add(0, pts.get(i));
                        //else
    ...
    public List<Point> getReversePoints() {
        return generateReversePath().pathPoints;
    }
    ...
    public Path generateReversePath() {
        Path p = new Path();
        p.pathPoints = pathPoints;
        Collections.reverse(p.pathPoints);
        p.goal = start;
        p.start = goal;
        p.found = found;
        p.length = length;
    ...
    private LinkedList<Point> neighbours(Point pt, int stepSize) {
        ...
        for(neighbourX=pt.x-stepSize; neighbourX<=pt.x+stepSize; neighbourX+=stepSize)
            for(neighbourY=pt.y-stepSize; neighbourY<=pt.y+stepSize; neighbourY+=stepSize) {
                ...
                //if(grid.obstacleAt(neighbourX, neighbourY) || !grid.freeSpaceAt(neighbourY, neighbourY))
                    continue;

                // Check 3: is location reachable
                if(!grid.directLinePossible(pt.x, pt.y, neighbourX, neighbourY) || !grid.directLinePossible(neighbourX, neighbourY, pt.x, pt.y))
                    continue;

                // Check 4: is it not too close to wall (unless it's a goal)
                if(grid.obstacleWithinDistancePath(neighbourX, neighbourY, Constants.WALL_DISTANCE) &&
                   !(goal.distance(neighbourX, neighbourY) <= Constants.STEP_SIZE )) //&&
                   //!(start.distance(neighbourX, neighbourY) <= Constants.STEP_SIZE))
                    continue;
    ...
    //Adds all points in list2 to list1 (no duplicates), returns merged list1 to the head.
