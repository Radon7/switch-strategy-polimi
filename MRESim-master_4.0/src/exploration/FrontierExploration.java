...
public class FrontierExploration {
    ...
    public static Point replan(RealAgent agent, SimulatorConfig.frontiertype frontierExpType, int timeElapsed, SimulatorConfig simConfig) {
        ...
        agent.setReplanTime(agent.getReplanTime() + System.currentTimeMillis() - realtimeStart);

        return nextStep;
    }

// <editor-fold defaultstate="collapsed" desc="Choose best frontier">
    private static LinkedList<Frontier> frontiersOfInterest(RealAgent agent, Frontier lastFrontier, LinkedList<Frontier> linkedList, OccupancyGrid grid) {
        PriorityQueue<Frontier> copy = new PriorityQueue();
        LinkedList<Frontier> list = new LinkedList();

        for (Frontier f : linkedList) {
            copy.add(f.copy());
        }
        ...
        for (int i = copy.size(); i > 0; i--) {
            ...
            if (currFrontier == lastFrontier) {
                ...
                Path pathToFrontier = agent.calculatePath(agent.getTeammate(1).getLocation(),
                        currFrontier.getCentre());
                if (!pathToFrontier.found  || //(currFrontier.getArea() < Constants.MIN_FRONTIER_SIZE * 4) && //(grid.obstacleWithinDistance(currFrontier.getCentre().x, currFrontier.getCentre().y, 1)))
                        grid.obstacleWithinDistancePath(currFrontier.getCentre().x, currFrontier.getCentre().y, 2))
                {} else {
                    list.add(currFrontier);
                    counter++;
                }
    ...
    private static double utilityEstimate(Point agentLoc, Frontier frontier) {
        return ((frontier.getArea() * 1000000000) / Math.pow(agentLoc.distance(frontier.getCentre()), 4));
    }
    ...
    private static void calculateUtilityExact(RealAgent agent, Utility ute) {
        ...
        if (p.found) {
            ute.path = p;
            ute.utility = (ute.frontier.getArea() * 1000000000) / Math.pow(p.getLength(), 4);
        } else {
            ute.utility = -1000;
        }
