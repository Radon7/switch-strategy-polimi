...
public class AgentStepRunnable implements Runnable{
    ...
    public void run() 
    {
    	if(agent.getRobotNumber() != Constants.BASE_STATION_ID) { //AGGIUNTO
    		Point nextStep = null;
    		double[] sensorData;
    		...
    		while (distance_left > 0)
    		{
    			...
    			try {
    				nextStep = agent.takeStep(timeElapsed, simConfig, null);
    			...
    			if(env.directLinePossible(agent.getX(), agent.getY(), nextStep.x, nextStep.y)) {
    				...
    				//agent.writeStep(nextStep, sensorData, env);
    			}
    			else
    			{
        	...
        	if (simConfig.getExpAlgorithm() == exptype.FrontierExploration && timeElapsed >= Constants.INIT_CYCLES) { //AGGIUNTO
        		/*try {
					//StumpExploration.replan(agent, frontiertype.ReturnWhenComplete, timeElapsed, simConfig);
				} catch (FileNotFoundException e) {
					e.printStackTrace();
				}*/
        	}
