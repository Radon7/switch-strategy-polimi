...
// <editor-fold defaultstate="collapsed" desc="Take Step">
    ...
    private static Point takeStep_Initial(RealAgent agent, SimulatorConfig simConfig) {
        ...
        if (agent.getStateTimer() < Constants.INIT_CYCLES)
    ...
    private static Point takeStep_Explore(RealAgent agent, SimulatorConfig simConfig) {  
        ...
        if (timeElapsed > Constants.INIT_CYCLES && (agent.getFrontiers().isEmpty()) || (agent.getPercentageKnown() >= Constants.TERRITORY_PERCENT_EXPLORED_GOAL)) { 
            ...
            if(agent.getPath().getPoints() != null && agent.getPath().getPoints().size() > 0) {
    ...
    private static Point takeStep_ReturnToParent(RealAgent agent, SimulatorConfig simConfig) { 
        ...
        if(agent.getTeammate(Constants.BASE_STATION_ID).isInRange()) {
            ...
            if ((SwitchExploration.getState() == SwitchExploration.SwitchState.RendezVous ||
                    ...
                    && timeElapsed>150){
                SwitchExploration.setState(SwitchExploration.SwitchState.Wait);
                return agent.getLocation();
            }
