...
def lazy_constraint(model, where):
    ...
    if((where == GRB.callback.MIPNODE and GRB.status.OPTIMAL == model.cbGet(GRB.callback.MIPNODE_STATUS) ) or where == GRB.callback.MIPSOL):
        ...
        Y = model._Y
        edges = model._edges
        frontiers = model._frontiers

        for edge in edges:
            if(where == GRB.callback.MIPNODE):
                G_W.es[G_W.get_eid(edge[0],edge[1])]['capacity'] = model.cbGetNodeRel(X[(edge[0],edge[1])])
            else:
                G_W.es[G_W.get_eid(edge[0],edge[1])]['capacity'] = model.cbGetSolution(X[(edge[0],edge[1])])
                #print "VAL " + str(x)
            #X_W[(edge[0],edge[1])] = x
            #G_W.es[G_W.get_eid(edge[0],edge[1])]['capacity'] = X_W[(edge[0],edge[1])]
        ...
        for i in frontiers:
            if y_val[i] < 1e-4: continue
            cut_number = 0
            added_constraint = True
            #Altrimenti va qui, non sotto
            
            G_cur = G_W.copy()
            while cut_number < MAXCUTS and added_constraint:
                added_constraint = False

                result = G_cur.maxflow(bs,i,capacity="capacity")#nx.minimum_cut(G_W, bs, i)
                
                if result.value + 1e-4 < y_val[i]:

                    if(bs in result.partition[0]):
                        partition_root = result.partition[0]
                    else:
                        partition_root = result.partition[1]
                    ...
                    for edge in cut_set:
                        G_cur.es[G_W.get_eid(edge[0],edge[1])]['capacity'] = 1.0
...
def addConstraintsDepl(m, G, TERMINALS, X, Y, edges, max_dim):
    ...
    #for edge in edges:
    #    if edge[0] == TERMINALS[0]: continue
    #    m.addConstr(X[edge[0], edge[1]] + X[edge[1], edge[0]] <= 1)
...
def setObjectiveDepl(m, Y, FRONTIERS, utilities, X, edges, max_dim):
    #print NORM
    m.setObjective(quicksum(utilities[FRONTIERS.index(fr)]*Y[fr] for fr in FRONTIERS), sense = GRB.MAXIMIZE)
    #m.setObjective(quicksum(utilities[FRONTIERS.index(fr)]*Y[fr] for fr in FRONTIERS) - ((num_eta)/float(max_dim))*quicksum(X[edge[0],edge[1]] for edge in edges), sense = GRB.MAXIMIZE)
...
def solve_bpcst(true_frontiers, bs, utilities, max_dim, G, out_arcs):
    ...
    setObjectiveDepl(m, Y, true_frontiers, utilities, X, edges, max_dim)
    ...
    G_W = Graph(directed=True)
    for v1 in range(G.shape[0]):
        G_W.add_vertex()

    for edge in edges:
        G_W.add_edge(edge[0],edge[1],capacity=1)
    ...
    if m.status == GRB.status.INF_OR_UNBD or m.status == GRB.status.INFEASIBLE:
        ...
        for edge in solution:
            if(edge[0] not in added_nodes):
                G_W.add_node(edge[0])
                added_nodes.add(edge[0])

            if(edge[1] not in added_nodes):
                G_W.add_node(edge[1])
        ...
        for edge in solution:
            if(edge[0] == bs):
                cleaned_solution.append(edge) 
                continue

            if(edge[0] not in nx.descendants(G_W, source = bs)):
                print "NOT IN DESCENDANTS:"
                print edge
                if edge[0] in selected_frontiers:
                    panic = True 
                continue

            if(edge[1] not in nx.descendants(G_W, source = bs)):
                print "NOT IN DESCENDANTS:"
                print edge
                if edge[1] in selected_frontiers: 
                    panic = True
                continue

            Gcopy = deepcopy(G_W)
            Gcopy.remove_edge(*edge)
            useful = False
            for f in true_frontiers:
                if(f not in nx.descendants(Gcopy, source = bs)):
                    useful = True
                    break
            ...
            if(not(useful)):
                ...
                print edge
                #potential bug
                G_W.remove_edge(*edge)
                #fuseless = open("useless.txt", "r")
                #n = int(fuseless.readlines()[0])
                #fuseless.close()
                #fuseless = open("useless.txt","w")
                #fuseless.write(str(n + 1))
                #fuseless.close()
                continue
        ...
        #print solution
        #IF the solution is not connected, log

        solution = deepcopy(cleaned_solution)
        ...
        print added_nodes
        for n in filter(lambda x: x!= bs, added_nodes):
            if(n not in nx.descendants(G_W, source = bs)):"""
        if panic:
            print "PANICO"
            #log
...
def run():
        ...
        for line in fct.readlines():
            ...
            if(int(s[0]) not in replanning_robots):
                ...
                if(int(s[0]) not in NOT_READY_COMM):
                    currentTreeOnlyNotComm.append((int(s[0]), int(s[1]) - 1))
    ...
    """if(len(true_frontiers) == 0):
        if(MIXED):
            fcmin = open(my_dir + "costMinApp.txt", "w")
            fcmin.write("Inf")
            fcmin.close()
    ...
    #utilities = map(lambda x: x*1000.0, utilities)
