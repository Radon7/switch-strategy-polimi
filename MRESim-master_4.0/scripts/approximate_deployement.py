...
def is_valid_sp(candidates, all_pair_sp, bs, max_dim):
    ...
    for pair in itertools.combinations(all_terminals, 2):
        ...
        cur_num = len(all_pair_sp[pair]) - 1 #as it may contain the bs!
...
def pad_solution(sol, nodes, num_available, G, frontiers):
    G_B = nx.Graph()
    ...
    while(num_available > 0 and len(free_frontiers)):
        ...
        best_path = None
        for node in nodes:
            if(cur_frontier in nx.descendants(G_B, source=node)):
                path = nx.shortest_path(G_B, source=node, target=cur_frontier)
            else:
                path = None            
            #print node
...
def run():
    ...
    #Added
    all_pair_sp = {}
    candidate_nodes = true_frontiers + [CUR_LOC[0]]
    G_B = nx.Graph()
    for v1 in range(G.shape[0]):
        G_B.add_node(v1)
    ...
    for v1 in candidate_nodes:
        for v2 in candidate_nodes:
    ...
    #no frontiers taken with all robots ready -> move up
    ...
    if len(nodes) < len(CUR_LOC) - 1:
        nodes = nodes + [bs]
        new_frontiers = pad_solution(sol, nodes, (len(CUR_LOC) - 1) - (len(nodes) - 1), G, true_frontiers)
