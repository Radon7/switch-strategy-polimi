...
def setObjectiveDepl(m, Y, FRONTIERS, utilities, X, edges):
    #print NORM
    m.setObjective(quicksum(utilities[FRONTIERS.index(fr)]*Y[fr] for fr in FRONTIERS) - ((min(utilities) - (min(utilities)/2.0))/len(edges))*quicksum(X[edge[0],edge[1]] for edge in edges), sense = GRB.MAXIMIZE)
...
def solve_bpcst(true_frontiers, bs, utilities, max_dim, G, out_arcs):
    ...
    setObjectiveDepl(m, Y, true_frontiers, utilities, X, edges)
    ...
    if m.status == GRB.status.INF_OR_UNBD or m.status == GRB.status.INFEASIBLE:
        ...
        for edge in edges:
            if (x[edge[0], edge[1]] > 0.5): #f[fr,v1,v2] > 0 and (v1,v2) not in solution and (v2,v1) not in solution): #edge
                ...
                print str(edge[0]) + "," + str(edge[1]) + ": " + str(x[edge[0], edge[1]])
...
if __name__ == "__main__":
    ...
    true_frontiers = [8,9,6]
    utilities = [1,1,200]
    bs = 0

    sol,a = solve_bpcst(true_frontiers, bs, utilities, 6, G, out_arcs)
